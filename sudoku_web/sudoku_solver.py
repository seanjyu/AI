...
def mrv(domain_dict, board):
    unassigned_tile = [tile for tile in domain_dict.keys() if
                       board[tile] == 0]
...
def checkdone(board):
    for i in ROW:
        for j in COL:
            if not check(board, board[i + j], i, j):
                return False
    return True
...
def domains(board):
    ...
    if len(allvars) > 0:
        ...
        orddict = {a: b for a, b in
                   sorted(varsdict.items(), key=lambda item: len(item[1]))}
        return orddict
